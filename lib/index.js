...
import { buildBasicAuthorization, buildJWTAuthorization, buildTauriRequestData, buildRequestUrl, getTauriResponseType, } from './util';
export const axiosTauriApiAdapter = (config) => new Promise(async (resolve, reject) => {
    ...
    client
        ...
        if (response.ok) {
            ...
            reject(new AxiosError('Request failed with status code ' + response.status, [AxiosError.ERR_BAD_REQUEST, AxiosError.ERR_BAD_RESPONSE][Math.floor(response.status / 100) - 4], config, client, {
                data: response.data,
                status: response.status,
                statusText: statusText,
